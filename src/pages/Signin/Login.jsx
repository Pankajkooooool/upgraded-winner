...
const Login = () => {
  ...
  const [display, setDisplay] = useState('100');
    ...
    async function handleLoginReq(Uid, password) {
        ...
        const apiUrl = 
        `${process.env.REACTAPPAPI}/auth/userlogin`  ;
  ...
  <div className="input_container">
    ...
    <input placeholder="yourgmail@gmail.com" title="Inpit title" name="input-name" type="text" className="input_field" id="email_field" />
  </div>
  <div className="input_container">
    ...
    <input placeholder="Password" title="Inpit title" name="password" type="password" className="input_field" id="password_field"  value={formData.password}
          onChange={handleChange} />
  </div>
  <button title="Sign In"  className="sign-in_btn" onClick={()=>{
    localStorage.setItem('auth',true);
    if(form.password!='tyui'){
